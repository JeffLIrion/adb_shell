...
class TestAdbDeviceAsync(unittest.TestCase):
    ...
    async def test_init_banner(self):
        ...
        device_with_banner2 = AdbDeviceAsync(
            transport=FakeTcpTransportAsync("host", 5555), banner=bytearray("banner2", "utf-8")
        )
        self.assertEqual(device_with_banner2._banner, b"banner2")

        device_with_banner3 = AdbDeviceAsync(transport=FakeTcpTransportAsync("host", 5555), banner="banner3")
    ...
    async def test_shell_multibytes_sequence_exceeds_max(self):
        ...
        self.transport.bulk_read_data = join_messages(
            AdbMessage(command=constants.OKAY, arg0=1, arg1=1, data=b"\x00"),
            AdbMessage(
                command=constants.WRTE, arg0=1, arg1=1, data=b"0" * (self.device.max_chunk_size - 1) + b"\xe3\x81\x82"
            ),
            AdbMessage(command=constants.CLSE, arg0=1, arg1=1, data=b""),
        )

        self.assertEqual(await self.device.shell("TEST"), "0" * (self.device.max_chunk_size - 1) + "\u3042")
    ...
    async def test_shell_with_multibytes_sequence_over_two_messages(self):
        ...
        self.assertEqual(await self.device.shell("TEST"), "\u3042")
    ...
    async def test_push_dir(self):
        ...
        with patch("aiofiles.open", async_mock_open(read_data=filedata)), patch(
