...
class _AdbIOManagerAsync(object):
    ...
    async def connect(self, banner, rsa_keys, auth_timeout_s, auth_callback, adb_info):
        ...
        async with self._transport_lock:
            ...
            cmd, arg0, maxdata, banner2 = await self._read_expected_packet_from_device(
                [constants.AUTH, constants.CNXN], adb_info
            )
            ...
            for rsa_key in rsa_keys:
                ...
                cmd, arg0, maxdata, banner2 = await self._read_expected_packet_from_device(
                    [constants.CNXN, constants.AUTH], adb_info
                )
    ...
    async def read(self, expected_cmds, adb_info, allow_zeros=False):
        ...
        async with self._store_lock:
            ...
            arg0_arg1 = (
                self._packet_store.find(adb_info.remote_id, adb_info.local_id)
                if not allow_zeros
                else self._packet_store.find_allow_zeros(adb_info.remote_id, adb_info.local_id)
            )
            while arg0_arg1:
        ...
        # Timeout
        raise exceptions.AdbTimeoutError(
            "Never got one of the expected responses: {} (transport_timeout_s = {}, read_timeout_s = {})".format(
                expected_cmds, adb_info.transport_timeout_s, adb_info.read_timeout_s
            )
        )
    ...
    async def _read_bytes_from_device(self, length, adb_info):
        ...
        while length > 0:
            ...
            if time.time() - start > adb_info.read_timeout_s:
                # Timeout
                raise exceptions.AdbTimeoutError(
                    "Timeout: read {} of {} bytes (transport_timeout_s = {}, read_timeout_s = {})".format(
                        len(data), len(data) + length, adb_info.transport_timeout_s, adb_info.read_timeout_s
                    )
                )
    ...
    async def _read_packet_from_device(self, adb_info):
        ...
        if not command:
            raise exceptions.InvalidCommandError(
                "Unknown command: %d = '%s' (arg0 = %d, arg1 = %d, msg = '%s')"
                % (cmd, int_to_cmd(cmd), arg0, arg1, msg)
            )
...
class AdbDeviceAsync(object):
    ...
    def __init__(self, transport, default_transport_timeout_s=None, banner=None):
        ...
        if not isinstance(transport, BaseTransportAsync):
            raise exceptions.InvalidTransportError(
                "`transport` must be an instance of a subclass of `BaseTransportAsync`"
            )
    ...
    async def connect(
        ...
        adb_info = _AdbTransactionInfo(
            None, None, self._get_transport_timeout_s(transport_timeout_s), read_timeout_s, None
        )
    ...
    async def _service(
        ...
        if decode:
            return b"".join(
                [
                    x
                    async for x in self._streaming_command(
                        service, command, transport_timeout_s, read_timeout_s, timeout_s
                    )
                ]
            ).decode("utf8", "backslashreplace")
        return b"".join(
            [x async for x in self._streaming_command(service, command, transport_timeout_s, read_timeout_s, timeout_s)]
        )

    async def _streaming_service(
        self, service, command, transport_timeout_s=None, read_timeout_s=constants.DEFAULT_READ_TIMEOUT_S, decode=True
    ):
    ...
    async def exec_out(
        ...
        if not self.available:
            raise exceptions.AdbConnectionError(
                "ADB command not sent because a connection to the device has not been established.  (Did you call `AdbDeviceAsync.connect()`?)"
            )

        return await self._service(
            b"exec", command.encode("utf8"), transport_timeout_s, read_timeout_s, timeout_s, decode
        )

    async def reboot(
        self, fastboot=False, transport_timeout_s=None, read_timeout_s=constants.DEFAULT_READ_TIMEOUT_S, timeout_s=None
    ):
        """Reboot the device.
        ...
        await self._open(
            b"reboot:bootloader" if fastboot else b"reboot:", transport_timeout_s, read_timeout_s, timeout_s
        )
    ...
    async def shell(
        ...
        return await self._service(
            b"shell", command.encode("utf8"), transport_timeout_s, read_timeout_s, timeout_s, decode
        )

    async def streaming_shell(
        self, command, transport_timeout_s=None, read_timeout_s=constants.DEFAULT_READ_TIMEOUT_S, decode=True
    ):
        ...
        async for line in self._streaming_service(
            b"shell", command.encode("utf8"), transport_timeout_s, read_timeout_s, decode
        ):
            yield line
    ...
    async def list(self, device_path, transport_timeout_s=None, read_timeout_s=constants.DEFAULT_READ_TIMEOUT_S):
        ...
        async for cmd_id, header, filename in self._filesync_read_until(
            [constants.DENT], [constants.DONE], adb_info, filesync_info
        ):
    ...
    async def _pull(self, device_path, stream, progress_callback, adb_info, filesync_info):
        ...
        async for cmd_id, _, data in self._filesync_read_until(
            [constants.DATA], [constants.DONE], adb_info, filesync_info
        ):
            if cmd_id == constants.DONE:
                break
    ...
    async def push(
        ...
        local_path,
        device_path,
        st_mode=constants.DEFAULT_PUSH_MODE,
        mtime=0,
        progress_callback=None,
        transport_timeout_s=None,
        read_timeout_s=constants.DEFAULT_READ_TIMEOUT_S,
        ...
        local_path_is_dir, local_paths, device_paths = await get_running_loop().run_in_executor(
            None, get_files_to_push, local_path, device_path
        )
    ...
    async def _push(self, stream, device_path, st_mode, mtime, progress_callback, adb_info, filesync_info):
        ...
        async for cmd_id, _, data in self._filesync_read_until(
            [], [constants.OKAY, constants.FAIL], adb_info, filesync_info
        ):
    ...
    async def _open(self, destination, transport_timeout_s, read_timeout_s, timeout_s):
        ...
        async with self._local_id_lock:
            ...
            adb_info = _AdbTransactionInfo(
                self._local_id, None, self._get_transport_timeout_s(transport_timeout_s), read_timeout_s, timeout_s
            )
    ...
    async def _filesync_flush(self, adb_info, filesync_info):
        ...
        # Send the buffer
        msg = AdbMessage(
            constants.WRTE, adb_info.local_id, adb_info.remote_id, filesync_info.send_buffer[: filesync_info.send_idx]
        )
        await self._io_manager.send(msg, adb_info)
